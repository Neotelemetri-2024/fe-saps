...
exports.getCatatanPA = exports.getRiwayatIzin = exports.ajukanIzinPA = void 0;
const prisma_1 = __importDefault(require("../../lib/prisma"));
const notifikasi_service_1 = require("../../services/notifikasi.service");
const ASAL_INTERNAL = ['kurikuler_ukm', 'kurikuler_ukmf', 'universitas'];
function isAsalInternal(asal) {
    return !!asal && ASAL_INTERNAL.includes(asal);
}
// 1. Mengajukan Izin PA (dari Riwayat Kegiatan Internal maupun Katalog)
const ajukanIzinPA = async (req, res, next) => {
    try {
        ...
        let targetPartisipasi = null;
        let targetKegiatan = null;
        if (partisipasiId) {
            targetPartisipasi = await prisma_1.default.partisipasi.findUnique({
            ...
            targetKegiatan = await prisma_1.default.kegiatan.findUnique({ where: { id: usedKegiatanId } });
            if (!targetKegiatan) {
            ...
            if (usedPeranId) {
                ...
                if (!klaim) {
                    ...
                    await tx.klaimPoin.update({
                        ...
                        data: {
                            peranUsulanId: usedPeranId,
                            ...(klaim.status === 'disetujui' ? {} : { status: 'draft' }),
                        }
                    });
                }
            }
...
exports.ajukanIzinPA = ajukanIzinPA;
// 2. Mengambil Riwayat Izin PA
const getRiwayatIzin = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        ...
        const riwayat = await prisma_1.default.izinPA.findMany({
            where: whereClause,
            include: {
                partisipasi: {
                    include: {
        ...
        const formattedData = riwayat
            .filter((item) => item.partisipasi && item.partisipasi.kegiatan)
            .map((item) => {
...
exports.getRiwayatIzin = getRiwayatIzin;
// 3. Mengambil Catatan Dosen PA (Saran PA)
const getCatatanPA = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        }
        const catatan = await prisma_1.default.saranPA.findMany({
            where: {
                mahasiswaId: BigInt(userId)
            },
            orderBy: {
        ...
        const formattedCatatan = catatan.map((c) => ({
            id: c.id.toString(),
            isi: c.isi,
...
exports.getCatatanPA = getCatatanPA;
