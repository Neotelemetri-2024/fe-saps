...
exports.getRiwayatKegiatanInternal = exports.getRiwayatPoin = exports.getDashboard = void 0;
exports.hitungProgresKurikulumMahasiswa = hitungProgresKurikulumMahasiswa;
...
function hitungProgresKurikulumMahasiswa(kurikulum, perolehanPoin) {
    const capaianMapById = {};
    const capaianMapByName = {};
    const subCapaianMapById = {};
    const subCapaianMapByName = {};
    ...
    const progresTahunan = kurikulum.capaian.map((c) => {
    ...
    const totalTarget = kurikulum.capaian.reduce((sum, c) => sum + c.jumlahPoin, 0);
    // Total poin yang masuk ke progres (akumulasi poinProgres yang di-cap)
    const totalPoinProgres = progresTahunan.reduce((sum, item) => sum + item.poinProgres, 0);
    ...
    const radarData = kurikulum.capaian.map((c) => {
        const prog = progresTahunan.find((p) => p.id === c.id);
        return {
            label: c.nama,
    ...
    const isLulus = totalTarget > 0 && totalPoinProgres >= totalTarget && progresTahunan.every((c) => c.poinProgres >= c.targetPoin);
...
const getDashboard = async (req, res, next) => {
    try {
        ...
        const kurikulumAktif = await (0, kurikulumResolver_service_1.resolveKurikulumMahasiswa)(mahasiswa);
        if (!kurikulumAktif) {
        ...
        const riwayatIzinPA = await prisma_1.default.izinPA.findMany({
            where: {
        ...
        const riwayatEksternal = await prisma_1.default.kegiatan.findMany({
            where: {
        ...
        if (persentaseTotal >= 75)
            tahap = 'Tahap IV: Akhir';
        else if (persentaseTotal >= 50)
            tahap = 'Tahap III: Mahir';
        else if (persentaseTotal >= 25)
            tahap = 'Tahap II: Menengah';
        res.status(200).json({
            success: true,
            data: {
...
const getRiwayatPoin = async (req, res, next) => {
    try {
        ...
        if (penyelenggara) {
            whereKlaim.partisipasi.kegiatan = {
                ...whereKlaim.partisipasi.kegiatan,
                penyelenggaraExt: { contains: penyelenggara }
            };
        }
        if (tahun) {
            const yearStart = new Date(`${tahun}-01-01`);
            const yearEnd = new Date(`${parseInt(tahun) + 1}-01-01`);
            whereKlaim.partisipasi.kegiatan = {
                ...whereKlaim.partisipasi.kegiatan,
        ...
        if (peranId) {
            whereKlaim.peranUsulanId = parseInt(peranId);
        }
        if (search) {
            whereKlaim.partisipasi.kegiatan = {
                ...whereKlaim.partisipasi.kegiatan,
        ...
        const klaimData = await prisma_1.default.klaimPoin.findMany({
            where: whereKlaim,
            include: {
                partisipasi: {
                    include: {
...
exports.getRiwayatPoin = getRiwayatPoin;
// ==================== RIWAYAT KEGIATAN INTERNAL ====================
const getRiwayatKegiatanInternal = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        ...
        if (search) {
            whereKegiatan.nama = { contains: search };
        }
        if (kategoriId) {
            whereKegiatan.kategoriId = parseInt(kategoriId);
        }
        if (tahun) {
            const yearStart = new Date(`${tahun}-01-01`);
            const yearEnd = new Date(`${parseInt(tahun) + 1}-01-01`);
        ...
        else if (kehadiran === 'belum')
            where.kehadiran = null;
        const partisipasi = await prisma_1.default.partisipasi.findMany({
            where,
            include: {
                kegiatan: {
                    include: {
        ...
        const perolehanFallback = kegiatanIds.length
            ? await prisma_1.default.perolehanPoin.findMany({
                where: {
                    mahasiswaId: BigInt(userId),
            ...
            : [];
        const poinByKegiatan = new Map(perolehanFallback.map((p) => [p.kegiatanId, p.totalPoin]));
        // Estimasi poin dari matriks untuk peserta yang sudah punya peran (belum tentu sudah cair)
        const estimasiEntries = await Promise.all(partisipasi
            ...
            return matriks ? [p.id.toString(), matriks.poin] : null;
        }));
        const estimasiByPartisipasi = new Map(estimasiEntries.filter(Boolean));
        const riwayat = partisipasi.map((p, i) => {
            let statusKehadiran = 'Belum Tercatat';
            if (p.kehadiran === true)
                statusKehadiran = 'Hadir';
            else if (p.kehadiran === false)
                statusKehadiran = 'Tidak Hadir';
            const izinTerbaru = p.izinPA?.[0] || null;
            let statusPa = 'belum_disetujui';
            ...
            const poinDariKlaim = p.klaimPoin?.perolehanPoin?.status === 'sah'
            ...
            if (izin) {
                if (izin.status === 'diajukan')
                    statusIzin = 'Menunggu Dosen PA';
                else if (izin.status === 'disetujui')
                    statusIzin = 'Disetujui';
                else if (izin.status === 'ditolak')
                    statusIzin = 'Ditolak';
                else if (izin.status === 'revisi')
                    statusIzin = 'Perlu Revisi';
            }
...
exports.getRiwayatKegiatanInternal = getRiwayatKegiatanInternal;
