...
exports.hapusKegiatan = exports.publikasiKegiatan = exports.approvalKegiatanBulk = exports.approvalKegiatan = exports.getKegiatanForApproval = exports.verifikasiKegiatan = exports.getKegiatanForVerifikasi = exports.ajukanKegiatan = exports.verifikasiKegiatanBulk = exports.editKegiatan = exports.createKegiatan = exports.getKegiatanById = exports.getAllKegiatan = void 0;
...
const createKegiatanSchema = zod_1.z.object({
    ...
    asal: zod_1.z.enum(['kurikuler_ukm', 'kurikuler_ukmf', 'universitas', 'eksternal', 'internal']),
    deskripsi: zod_1.z.string().max(500).optional(),
    tanggalMulai: zod_1.z.string().refine(v => !isNaN(Date.parse(v))),
    tanggalSelesai: zod_1.z.string().refine(v => !isNaN(Date.parse(v))),
    lokasi: zod_1.z.string().max(200).optional(),
...
const approvalSchema = zod_1.z.object({
    ...
    alokasi: zod_1.z.array(zod_1.z.object({
        subCapaianId: zod_1.z.number(),
        alokasiPersen: zod_1.z.number().min(0).max(100),
    })).optional(),
});
const PENYELENGGARA_ADMIN_DITMAWA = 'Direktorat Kemahasiswaan UNAND';
async function resolvePenyelenggaraAdmin(effectiveRole, userId, organisasiId, explicit) {
    if (explicit?.trim())
    ...
    try {
        ...
        if (userJabatan === 'admin_fakultas' && !asal) {
            ...
            if (staffData?.fakultasId) {
                const adminsFakultas = await prisma_1.default.staff.findMany({
...
const getKegiatanById = async (req, res) => {
    try {
        ...
        const kurikulumNama = data.kurikulum?.nama ||
            data.kegiatanCapaian?.[0]?.subCapaian?.capaian?.kurikulum?.nama ||
            data.pembuat?.mahasiswa?.kurikulum?.nama ||
            (await prisma_1.default.kurikulum.findFirst({ where: { status: 'aktif', deletedAt: null }, select: { nama: true } }))?.nama ||
            null;
        const resData = {
            ...data,
            kurikulumNama,
...
const createKegiatan = async (req, res) => {
    try {
        ...
        try {
            kurikulumAktifList = await (0, kurikulumResolver_service_1.assertAlokasiCoversActiveKurikulum)(body.alokasi);
        }
        catch (err) {
        ...
        let resolvedAsal = body.asal === 'internal' ? 'kurikuler_ukm' : body.asal;
        ...
        const resolvedPenyelenggaraExt = await resolvePenyelenggaraAdmin(effectiveRole, dibuatOleh, resolvedOrganisasiId, body.penyelenggaraExt);
...
const editKegiatan = async (req, res) => {
    try {
        ...
        const userJabatan = req.user.jabatan;
        const effectiveRole = userPeran === 'staff' && userJabatan ? userJabatan : userPeran;
        const existing = await prisma_1.default.kegiatan.findUnique({
            where: { id: Number(id) }
        });
        if (!existing) {
            res.status(404).json({ success: false, message: 'Kegiatan tidak ditemukan' });
        ...
        if (userPeran === 'operator_org') {
            const operatorData = await prisma_1.default.organisasiOperator.findUnique({ where: { userId } });
        ...
        // Validasi alokasi: setiap kurikulum aktif harus tepat 100% [BR-032]
        try {
            await (0, kurikulumResolver_service_1.assertAlokasiCoversActiveKurikulum)(body.alokasi);
        }
        catch (err) {
            if (err instanceof kurikulumResolver_service_1.CurriculumResolutionError) {
                res.status(400).json({ success: false, message: err.message });
        ...
        const resolvedAsal = body.asal === 'internal'
            ? (existing.asal === 'kurikuler_ukmf' ? 'kurikuler_ukmf' : 'kurikuler_ukm')
            : body.asal;
        const resolvedPenyelenggaraExt = (await resolvePenyelenggaraAdmin(effectiveRole, userId, existing.organisasiId, body.penyelenggaraExt ?? existing.penyelenggaraExt)) ?? existing.penyelenggaraExt ?? undefined;
...
exports.editKegiatan = editKegiatan;
// PUT /api/kegiatan/verifikasi-bulk â€” Verifikasi banyak kegiatan sekaligus
const verifikasiKegiatanBulk = async (req, res, next) => {
    try {
        const aktorId = BigInt(req.user.id);
        const userJabatan = req.user.jabatan;
        const schema = zod_1.z.object({
            kegiatanIds: zod_1.z.array(zod_1.z.number()).min(1),
            keputusan: zod_1.z.enum(['setuju', 'tolak', 'revisi']),
            alasan: zod_1.z.string().optional(),
            alokasiBulk: zod_1.z.array(zod_1.z.object({
                kegiatanId: zod_1.z.number(),
                alokasi: zod_1.z.array(zod_1.z.object({
                    subCapaianId: zod_1.z.number(),
                    alokasiPersen: zod_1.z.number().min(0.01).max(100),
                })).min(1),
        ...
        const kegiatans = await prisma_1.default.kegiatan.findMany({
            where: {
        ...
        if (body.keputusan === 'setuju') {
            ...
            const tanpaPemetaan = kegiatans.filter((k) => k.asal === 'eksternal' && !alokasiMap.has(k.id));
        ...
        if (body.keputusan === 'revisi')
            statusBaru = 'perlu_revisi';
        if (body.keputusan === 'tolak')
            statusBaru = 'ditolak';
        ...
        for (const kegiatan of kegiatans) {
            ...
            if (body.keputusan === 'setuju' && alokasi && alokasi.length > 0) {
                await prisma_1.default.kegiatanCapaian.deleteMany({ where: { kegiatanId: kegiatan.id } });
                await prisma_1.default.kegiatanCapaian.createMany({
                    data: alokasi.map((a) => ({
            ...
            if (body.keputusan === 'setuju' && kegiatan.asal !== 'eksternal') {
                ...
                if (pimpinanTargets.length > 0) {
                    await notifikasi_service_1.NotifikasiService.kirimBatch({
...
exports.verifikasiKegiatanBulk = verifikasiKegiatanBulk;
...
const ajukanKegiatan = async (req, res) => {
    try {
        ...
        const isAdmin = effectiveRole === 'admin_ditmawa' || effectiveRole === 'admin_fakultas' || isSuperAdmin;
        if (userPeran === 'operator_org') {
            const operatorData = await prisma_1.default.organisasiOperator.findUnique({ where: { userId: aktorId } });
        ...
        if (isAdmin) {
            if (effectiveRole === 'admin_ditmawa') {
                ...
                const adminData = await prisma_1.default.staff.findUnique({ where: { userId: aktorId } });
                const pimpinanFakultas = await prisma_1.default.staff.findMany({
                    where: {
...
const verifikasiKegiatan = async (req, res) => {
    try {
        ...
        if (body.keputusan === 'setuju' && body.alokasi && body.alokasi.length > 0) {
            await prisma_1.default.kegiatanCapaian.deleteMany({ where: { kegiatanId: Number(id) } });
            await prisma_1.default.kegiatanCapaian.createMany({
                data: body.alokasi.map((a) => ({
                    kegiatanId: Number(id),
                    subCapaianId: a.subCapaianId,
...
const bulkApprovalSchema = zod_1.z.object({
    kegiatanIds: zod_1.z.array(zod_1.z.number().int().positive()).min(1, 'Minimal 1 kegiatan'),
    keputusan: zod_1.z.enum(['setuju', 'revisi', 'tolak']),
    alasan: zod_1.z.string().max(500).optional(),
});
const approvalKegiatanBulk = async (req, res, next) => {
    try {
        const aktorId = BigInt(req.user.id);
        const userJabatan = req.user.jabatan;
        ...
        for (const kegiatanId of body.kegiatanIds) {
            try {
                ...
                if (!kegiatan) {
                    errors.push(`ID ${kegiatanId}: tidak ditemukan`);
                    continue;
                }
                if (kegiatan.status !== 'terverifikasi') {
                    errors.push(`ID ${kegiatanId}: belum diverifikasi Admin`);
                    continue;
                }
                if (userJabatan === 'pimpinan_fakultas') {
                    const staffData = await prisma_1.default.staff.findUnique({ where: { userId: aktorId } });
                ...
                await prisma_1.default.kegiatan.update({ where: { id: kegiatanId }, data: { status: statusBaru } });
                await notifikasi_service_1.NotifikasiService.kirim({
                    userId: kegiatan.dibuatOleh,
                    judul: `Kegiatan ${body.keputusan === 'setuju' ? 'Disetujui' : body.keputusan === 'revisi' ? 'Perlu Revisi' : 'Ditolak'}`,
                    isi: `Kegiatan "${kegiatan.nama}" telah ${body.keputusan} oleh Pimpinan.${body.alasan ? ` Alasan: ${body.alasan}` : ''}`,
...
exports.approvalKegiatanBulk = approvalKegiatanBulk;
