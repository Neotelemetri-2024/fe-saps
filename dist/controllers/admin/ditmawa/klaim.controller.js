...
const getKlaimById = async (req, res) => {
    try {
        ...
        const kurikulumNama = data.perolehanPoin?.kurikulum?.nama ||
...
const validasiKlaim = async (req, res) => {
    try {
        ...
        if (body.keputusan === "disetujui") {
            ...
            const { kurikulum, matriks } = await (0, kurikulumResolver_service_1.resolveMatriksMahasiswa)(klaim.partisipasi.mahasiswaId, { kategoriId: kegiatan.kategoriId, skalaId: kegiatan.skalaId, peranId: peranFinalId });
            if (!matriks) {
                res.status(404).json({
                    success: false,
            ...
            const detailData = await (0, kurikulumResolver_service_1.buildSettlementDetails)(kegiatan.id, kurikulum.id, matriks.poin);
        ...
        await prisma_1.default.$transaction(async (tx) => {
            for (const klaim of klaims) {
                if (body.keputusan === "disetujui") {
                    ...
                    let matriks;
                    try {
                        ({ kurikulum, matriks } = await (0, kurikulumResolver_service_1.resolveMatriksMahasiswa)(klaim.partisipasi.mahasiswaId, { kategoriId: kegiatan.kategoriId, skalaId: kegiatan.skalaId, peranId: peranFinalId }, tx));
                    }
                    catch (err) {
                    ...
                    let bulkDetailData;
                    try {
                        bulkDetailData = await (0, kurikulumResolver_service_1.buildSettlementDetails)(kegiatan.id, kurikulum.id, matriks.poin, tx);
                    }
                    catch (err) {
