...
exports.hapusAkunUKMF = exports.resetPasswordUKMF = exports.toggleStatusAkunUKMF = exports.createAkunUKMF = exports.getAkunUKMF = void 0;
...
const createAkunLengkapSchema = zod_1.z.object({
    ...
    fakultasId: zod_1.z.coerce.number().optional().nullable(),
});
...
const createAkunUKMF = async (req, res) => {
    try {
        ...
        if (userRole === 'admin_fakultas') {
            ...
            const firstFakultas = await prisma_1.default.fakultas.findFirst({ select: { id: true } });
...
exports.toggleStatusAkunUKMF = toggleStatusAkunUKMF;
// PUT /api/organisasi-fakultas/akun/:userId/reset-password — Reset password akun UKMF
const resetPasswordUKMF = async (req, res) => {
    try {
        const { userId } = req.params;
        const aktorId = BigInt(req.user.id);
        const userRole = req.user.peran === 'staff' && req.user.jabatan ? req.user.jabatan : req.user.peran;
...
exports.resetPasswordUKMF = resetPasswordUKMF;
// DELETE /api/organisasi-fakultas/akun/:userId — Hapus Akun & UKMF
const hapusAkunUKMF = async (req, res) => {
    try {
        const { userId } = req.params;
        const aktorId = BigInt(req.user.id);
        const userRole = req.user.peran === 'staff' && req.user.jabatan ? req.user.jabatan : req.user.peran;
...
exports.hapusAkunUKMF = hapusAkunUKMF;
