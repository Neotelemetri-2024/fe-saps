...
exports.getDashboardFakultas = void 0;
const prisma_1 = __importDefault(require("../../../lib/prisma"));
// GET /api/dashboard/admin-fakultas — Dashboard Admin Fakultas
const getDashboardFakultas = async (req, res) => {
    try {
        ...
        let namaFakultas = staff?.fakultas?.nama || 'Fakultas';
        if (isSuperAdmin) {
            const targetFakId = req.query.fakultasId ? Number(req.query.fakultasId) : undefined;
            const targetFak = targetFakId
                ? await prisma_1.default.fakultas.findUnique({ where: { id: targetFakId } })
                : await prisma_1.default.fakultas.findFirst();
            if (targetFak) {
                fakultasId = targetFak.id;
                namaFakultas = targetFak.nama;
            }
        }
        else if (!staff || !staff.fakultasId) {
            res.status(403).json({ success: false, message: 'Akses Ditolak: Anda bukan Admin Fakultas' });
            return;
        }
        if (!fakultasId) {
            res.status(404).json({ success: false, message: 'Data fakultas tidak ditemukan' });
            return;
        }
        ...
        const tabelRiwayat = riwayatTerbaru.map((k) => {
            ...
            if (k.status === 'disetujui' || k.status === 'terpublikasi')
                ...
                statusStr = 'Draft';
            else if (k.status === 'terverifikasi')
                statusStr = 'Verifikasi Admin';
            return {
                id: k.id,
        ...
        const prodis = await prisma_1.default.programStudi.findMany({
            where: { fakultasId: fakultasId },
            select: { id: true, nama: true }
        });
        const grafikData = await Promise.all(prodis.map(async (prodi) => {
            ...
            const perolehans = await prisma_1.default.perolehanPoin.findMany({
                where: {
...
exports.getDashboardFakultas = getDashboardFakultas;
