...
exports.dashboardPimpinanDitmawa = void 0;
...
const dashboardPimpinanDitmawa = async (req, res) => {
    try {
        ...
        const [mahasiswaAktifCount, totalFakultasCount, totalOrmawaCount, antreanProposalKegiatanCount, antreanKlaimCount, poinSahAggregate, mahasiswaDenganPoin, kegiatanPendingList, klaimPendingList, ukmList, fakultasList,] = await Promise.all([
            // Total Mahasiswa Aktif
            prisma_1.default.mahasiswa.count({ where: { user: { aktif: true } } }),
            // Total Fakultas
            prisma_1.default.fakultas.count(),
            // Total Ormawa / UKM
            prisma_1.default.organisasi.count(),
            // Antrean Proposal Kegiatan (status 'diajukan' atau 'terverifikasi')
            prisma_1.default.kegiatan.count({ where: { status: { in: ['diajukan', 'terverifikasi'] } } }),
            // Antrean Klaim Poin (status 'menunggu_validasi' atau 'menunggu_pimpinan')
            prisma_1.default.klaimPoin.count({ where: { status: { in: ['menunggu_validasi', 'menunggu_pimpinan'] } } }),
            // Total Akumulasi Poin Sah
            prisma_1.default.perolehanPoin.aggregate({
                where: { status: 'sah' },
                _sum: { totalPoin: true },
            }),
            // Mahasiswa dengan Perolehan Poin untuk menghitung persentase kelulusan target
            prisma_1.default.mahasiswa.findMany({
                select: {
                    userId: true,
                    perolehanPoin: {
                        where: { status: 'sah' },
            ...
            prisma_1.default.kegiatan.findMany({
            ...
            prisma_1.default.klaimPoin.findMany({
            ...
            prisma_1.default.organisasi.findMany({
                where: { tipe: 'UKM' },
                select: { id: true, nama: true },
            }),
            // Daftar Fakultas untuk Top 5 Fakultas Teraktif
            prisma_1.default.fakultas.findMany({
                include: {
                    programStudi: {
                        include: {
                            mahasiswa: {
        ...
        const capaianTahunPoinMap = { 1: 0, 2: 0, 3: 0, 4: 0 };
        ...
        const persentaseLulusTarget = Math.min(Math.round((lulusTargetCount / totalMahasiswaTerhitung) * 100), 100);
        ...
        const grafikPoinUkm = await Promise.all(ukmList.map(async (ukm) => {
            const perolehans = await prisma_1.default.perolehanPoin.aggregate({
                where: {
                    status: 'sah',
        ...
        let iku3Widget = null;
        try {
            const iku3Data = await (0, iku3Calculation_service_1.calculateIku3Dashboard)({ tahun: new Date().getFullYear() });
            iku3Widget = {
