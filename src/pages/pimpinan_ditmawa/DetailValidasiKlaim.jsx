...
function DetailValidasiKlaim() {
  ...
  if (!item) return (
      <div className="flex flex-col items-center gap-4 py-20">
        <p className="text-base font-semibold text-base-content/60">Data klaim tidak ditemukan.</p>
  ...
  return (
      ...
      <Modal isOpen={showActionModal} onClose={() => !submitting && setShowActionModal(false)} size="md">
        <div className="space-y-4">
          <div>
            <h3 className="text-base font-bold text-base-content">{actionType === 'revisi' ? 'Minta Revisi' : 'Tolak Klaim'}</h3>
            <p className="mt-0.5 text-sm text-base-content/60">{actionType === 'revisi' ? 'Tuliskan catatan yang perlu diperbaiki.' : 'Tuliskan alasan penolakan klaim.'}</p>
          </div>
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-xl font-extrabold text-base-content sm:text-2xl">Detail Klaim Poin</h2>
            <p className="mt-1 text-sm text-base-content/60">Tinjau informasi klaim sebelum memberikan keputusan.</p>
          </div>
        ...
        {!canAct && !item.alasan && (
          <div className="rounded-xl border border-base-300 bg-base-200 px-5 py-3.5 flex items-center gap-2">
            ...
            <p className="text-sm text-base-content/60">Klaim sudah diverifikasi dengan status <StatusBadge status={item.status} /></p>
          </div>
        )}
        ...
        {canAct && (
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="button" onClick={() => setShowConfirmSetujui(true)}
              className="btn btn-primary px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:opacity-90">Setujui Klaim
            </button>
          </div>
        )}
      </div>
