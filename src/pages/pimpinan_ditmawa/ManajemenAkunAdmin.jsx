...
export default function ManajemenAkunAdmin() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.success) {
        setData(res.data.filter(d => d.jabatan === 'admin_ditmawa'))
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Manajemen Akun Admin Ditmawa</h2>
        <p className="text-base-content/70">Kelola akun Admin Tingkat Universitas</p>
      </div>

      <TableCard
        title="Daftar Akun Admin Ditmawa"
        headerRight={
          <button onClick={() => {
            setEditItem(null)
            setFormData({ nama: '', jabatan: 'admin_ditmawa', email: '', password: '', aktif: true })
            setIsModalOpen(true)
        ...
        <TableFrame>
          <DataTable
            columns={[
              ...
              { key: 'jabatan', label: 'Role', render: () => 'Admin Ditmawa' },
