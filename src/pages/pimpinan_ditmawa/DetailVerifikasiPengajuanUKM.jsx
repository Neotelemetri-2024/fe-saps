...
import { ArrowLeft, BookOpen, CalendarDays, CheckCircle2, FileText, ImageIcon, RotateCcw, XCircle } from 'lucide-react'
...
function normalizeDetail(raw) {
  ...
  ;(raw.kegiatanCapaian || []).forEach((kc) => {
    ...
    if (kc.subCapaian?.nama) subCapaianList.push({ label: kc.subCapaian.nama, persen: `${kc.alokasiPersen ?? kc.persen ?? 0}%` })
  })
  return {
    id: raw.id,
    kegiatan: raw.nama || '-',
...
function DetailVerifikasiPengajuanUKM() {
  ...
  useEffect(() => {
    ...
    getKegiatanById(id)
      ...
      .catch((err) => {
        if (location.state?.item) setItem(normalizeDetail({ ...location.state.item, nama: location.state.item.kegiatan, organisasi: { nama: location.state.item.namaUkm } }))
      ...
      <div className="space-y-5">
        ...
        {item.subCapaian?.length > 0 && (
          <SectionCard title="Sub Capaian & Bobot">
            {item.subCapaian.map((sc, i) => <InfoRow key={i} label={sc.label} value={sc.persen || `${sc.poin || 0}%`} />)}
          </SectionCard>
        )}
