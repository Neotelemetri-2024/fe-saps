...
import { Check, Filter, Search, Clock } from 'lucide-react'
...
const DUMMY_INTERNAL = [
  { id: 'int-1', namaMahasiswa: 'AUFA ALATA', nim: '2310512011', prodi: 'Teknik Komputer, S1', kegiatan: 'TAC (Training Andalasian Character)', kategori: 'Pelatihan', peran: 'Peserta', tanggal: '12 Feb - 14 Feb 2026', skala: 'lokal', status: 'pending', diajukanPada: 'Selasa, 4 Feb 2026, 15:37' },
  { id: 'int-2', namaMahasiswa: 'BUDI SANTOSO', nim: '2310512012', prodi: 'Sistem Informasi, S1', kegiatan: 'Seminar Kepemimpinan UKM', kategori: 'Seminar', peran: 'Panitia', tanggal: '20 Feb 2026', skala: 'lokal', status: 'pending', diajukanPada: 'Rabu, 5 Feb 2026, 10:00' },
  { id: 'int-3', namaMahasiswa: 'CITRA DEWI', nim: '2310512013', prodi: 'Teknik Elektro, S1', kegiatan: 'Latihan Dasar Organisasi', kategori: 'Organisasi', peran: 'Peserta', tanggal: '1 Mar 2026', skala: 'lokal', status: 'diteruskan', diajukanPada: 'Kamis, 6 Feb 2026, 09:15' },
]

function VerifikasiPengajuanInternal() {
  const navigate = useNavigate()
  const [items] = useState(DUMMY_INTERNAL)
  ...
  const getStatus = (item) => (approvedItems.has(item.id) ? 'diteruskan' : item.status)
            ...
            <div className="flex items-center gap-3 rounded-lg border border-[#e9ebf8] bg-[#f9fafb] px-4 py-3">
              ...
              <div className="ml-auto flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (selected.size === 0) { toast.error('Pilih minimal satu.'); return }
                    ...
                    <th className="px-4 py-3">
                      <button
                        ...
                        className={`flex h-5 w-5 items-center justify-center rounded border-2 transition ${
                          allPageSelected ? 'border-white bg-white' : 'border-white/70 hover:bg-white/20'
                        }`}
                      >
              ...
              <tbody>
                {pageItems.length === 0 ? (
                  ...
                  pageItems.map((item, index) => (
                    <tr
                      key={item.id}
                      className={`border-b border-[#e9ebf8] last:border-0 ${
                        selected.has(item.id) ? 'bg-green-50' : 'hover:bg-[#f9fafb]'
                      }`}
                    >
                      {pilihanMode && (
                          ...
                          <button
                            ...
                            className={`flex h-5 w-5 items-center justify-center rounded border-2 transition ${
                              selected.has(item.id)
                                ...
                                : 'border-[#c4c6cf] bg-white hover:border-brand-dark'
                            }`}
                          >
                        ...
                        {pilihanMode ? (
                          <button
                            ...
                            className={`flex h-6 w-6 items-center justify-center rounded border-2 transition ${
                              selected.has(item.id)
                                ? 'border-brand-dark bg-brand-dark text-white'
                                : 'border-[#c4c6cf] text-transparent'
                            }`}
                          >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              navigate(`/admin-ditmawa/verifikasi-pengajuan-internal/${item.id}`, {
                                state: { item },
                              })
                            }
          ...
          <div className="flex flex-col gap-3 border-t border-[#e9ebf8] px-4 py-3 text-xs text-[#616161] sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="rounded px-2 py-1 hover:bg-[#f0f4f0] disabled:opacity-40"
              >
                Previous
              </button>
