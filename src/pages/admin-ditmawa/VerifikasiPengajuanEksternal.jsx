...
import { getPengajuanEksternal, subscribeDataUpdate, teruskanKePimpinanDitmawa } from '../../services/pengajuanService'
      ...
      <div className="space-y-5">
        ...
        <div className="overflow-hidden rounded-xl border border-[#e9ebf8] bg-white shadow-sm">
            ...
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-brand-dark to-brand-light text-xs font-semibold uppercase tracking-wide text-white">
                  ...
                  {pilihanMode && <th className="px-4 py-3">Peran</th>}
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={pilihanMode ? 9 : 7} className="px-4 py-10 text-center text-[#616161]">
                      Memuat data...
                    </td>
                  </tr>
                ) : pageItems.length === 0 ? (
                  <tr>
                    <td colSpan={pilihanMode ? 9 : 7} className="px-4 py-10 text-center text-[#616161]">
                      Belum ada pengajuan. Data muncul setelah mahasiswa mengajukan kegiatan eksternal.
                    </td>
                  </tr>
                ) : (
                  pageItems.map((item, index) => (
                    ...
                    >
                      ...
                      {pilihanMode && (
                        <td className="px-4 py-3 text-[#616161]">{item.peran || 'Juara 1'}</td>
                      )}
                      ...
                      <td className="px-4 py-3">
                        {pilihanMode ? (
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              navigate(`/admin-ditmawa/verifikasi-pengajuan-eksternal/${item.id}`, {
                                state: { item },
                              })
                            }
          ...
          <div className="flex flex-col gap-3 border-t border-[#e9ebf8] px-4 py-3 text-xs text-[#616161] sm:flex-row sm:items-center sm:justify-between">
            <span>
              Showing {showingFrom} - {showingTo} From Total {filtered.length}
            </span>
            <span>
