...
function ManajemenAkunUKMF() {
  ...
  const handleSave = async (form) => {
    try {
      ...
      setShowTambah(false)
      toast.success('Akun UKMF berhasil dibuat!', { description: `${form.namaUkm} (${form.username})` })
      loadData()
    } catch (err) {
  ...
  return (
    <DashboardLayout role="admin_fakultas" userName={user?.nama || 'Admin Fakultas'} userRole="Admin Fakultas">
      ...
      <div className="space-y-5">
        <div>
          <h2 className="text-2xl font-bold text-brand-dark">Manajemen Akun UKMF</h2>
          <p className="mt-1 text-sm text-[#616161]">Kelola daftar akun UKMF di fakultas.</p>
        </div>
        ...
        <div>
          <h3 className="mb-3 text-lg font-bold text-brand-dark">Akun UKMF yang telah dibuat</h3>
              ...
              <table className="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-10 text-center text-[#616161]">Tidak ada data UKMF.</td>
                    </tr>
                  ) : (
