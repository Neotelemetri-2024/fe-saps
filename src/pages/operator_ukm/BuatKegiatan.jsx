...
function BuatKegiatan() {
  ...
  useEffect(() => {
    ...
    const existingKurikulumIds = [
      ...new Set(
        (editItem.kegiatanCapaian || editItem.alokasi || [])
          .map((a) => a.subCapaian?.capaian?.kurikulumId)
          .filter(Boolean)
      ),
    ]
    const fallbackKurId = editItem.kurikulumId ? [editItem.kurikulumId] : []
      ...
      <div className="space-y-5">
        ...
        <form onSubmit={handleSimpanDraft} className="space-y-5">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            <button
              type="button"
              onClick={() => navigate('/operator_ukm/daftar-kegiatan')} className={batalBtnClass}>
              Batal
            </button>
          </div>
