...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      const w = Math.max(rect.width, parentRect?.width ?? 0, container.clientWidth, 1)
      const h = Math.max(rect.height, parentRect?.height ?? 0, container.clientHeight, 1)
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        isVisible = entry.isIntersecting || entry.boundingClientRect.height > 1
