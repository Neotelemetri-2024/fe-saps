...
export const getKlaimForValidasi = async (req: Request, res: Response) => {
  try {
    const { validatorId, status, asal } = req.query;
    const where: any = {};

    // Jika validatorId dikirim, cari yang ditugaskan ke dia.
    // Jika Admin Ditmawa mencari eksternal, biasanya validatorId belum terisi (null/global)
