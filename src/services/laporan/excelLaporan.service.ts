...
export async function generateExcelLaporan(data: LaporanDataResult): Promise<Buffer> {
  ...
  const parts = [
    ...
    section(
      'Kurikulum',
      ['Tahapan', 'Tahun', 'Target', 'Rata-rata', 'Capaian'],
      data.capaianKurikulumStats.map((c) => [
        c.nama,
        c.tahun,
        c.targetPoin,
        c.rataRataTerkumpul,
    ...
    section(
      `Peringkat ${unitLabel}`,
      ['No', unitLabel, 'Mahasiswa', 'Rata-rata', 'Capaian'],
    ...
    section(
      'Mahasiswa',
      ['No', 'NIM', 'Nama', 'Prodi', 'Fakultas', 'Angkatan', 'Poin', 'Target', 'Capaian', 'Status'],
      data.mahasiswaList.map((m, i) => [
        i + 1,
        m.nim,
        m.nama,
        m.prodi,
    ...
    section(
      'Prestasi',
      ['No', 'Mahasiswa', 'NIM', 'Kegiatan', 'Skala', 'Peringkat', 'Poin', 'Tanggal'],
    ...
    section(
      'Ormawa',
      ['No', 'Organisasi', 'Tipe', 'Fakultas', 'Kegiatan', 'Partisipasi', 'Poin'],
      data.ormawaList.map((o, i) => [
        i + 1,
        o.nama,
        o.tipe,
        o.fakultas,
